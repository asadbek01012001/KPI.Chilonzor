      ...
      <TabPage
        headerCompoonent={
          <div className="d-flex justify-content-between align-items-center">
            <AppButton variant="secondary" label={`"Excel" дан юклаш`} className="px-4" onClick={() => setIsExcelOpen(true)} />
            <div className="d-flex gap-2">
              <DatePicker placeholder="Бошланғич сана" value={startDate} onChange={setStartDate} />
              <DatePicker placeholder="Охирги сана"    value={endDate}   onChange={setEndDate} />
            </div>
          </div>
        }
      >
        <CrimesTable data={data} totals={totals} handleBlur={(v) => console.log(v)} />
      </TabPage>

      {isExcelOpen && (
        <CrimesExcelReader onClose={() => setIsExcelOpen(false)} onSaved={loadData} />
      )}
    </>
  );
}
