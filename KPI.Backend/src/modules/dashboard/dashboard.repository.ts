...
export const dashboardRepository = {
  ...
  getAllMahalla: async (from?: string, to?: string, sector?: number) => {
    ...
    if (from) dateConds.push(`drs.date >= $${params.push(from)}`);
    if (to)   dateConds.push(`drs.date <= $${params.push(to)}`);
  ...
  topCrimes: async (from?: string, to?: string) => {
    const params: any[] = [];
    const dateConds: string[] = [];
    if (from) dateConds.push(`c.date >= $${params.push(from)}`);
    if (to)   dateConds.push(`c.date <= $${params.push(to)}`);
    const dateWhere = dateConds.length ? dateConds.join(' AND ') : 'TRUE';
  ...
  top102: async (from?: string, to?: string) => {
    const params: any[] = [];
    const dateConds: string[] = [];
    if (from) dateConds.push(`e.date >= $${params.push(from)}`);
    if (to)   dateConds.push(`e.date <= $${params.push(to)}`);
    const dateWhere = dateConds.length ? dateConds.join(' AND ') : 'TRUE';
  ...
  sectorRegions: async (from?: string, to?: string) => {
    ...
    return Object.values(sectorMap).sort((a: any, b: any) => a.sector - b.sector);
  },
