...
export const indicatorValueController = {
  ...
  deleteByDate: handle(async (req, res) => {
    ...
    if (!date) { res.status(422).json({ success: false, message: 'date query param required' }); return; }
    const result = await indicatorValueService.deleteByDate(date);
    sendSuccess(res, result, `Deleted ${result.deleted} indicator values for ${date}`);
  }),
};