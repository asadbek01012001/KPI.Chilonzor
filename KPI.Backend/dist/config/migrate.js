...
const migrate = async () => {
  ...
  `);
    ...
    const existing = await database_1.db.query(`SELECT id FROM users WHERE email = 'admin@kpi.uz' LIMIT 1`);
    if (!existing.rows.length) {
        const hash = await bcryptjs_1.default.hash('Admin@123', 10);
        await database_1.db.query(`INSERT INTO users (name, email, password_hash, role, is_active)
       VALUES ($1, $2, $3, $4, $5)`, ['Admin', 'admin@kpi.uz', hash, 'admin', true]);
        logger_1.logger.info('Seed admin created → email: admin@kpi.uz  password: Admin@123');
    }
