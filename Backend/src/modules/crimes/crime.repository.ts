...
export const crimeRepository = {
  findAll: async (q: CrimePaginationDtoType): Promise<{ rows: any[]; total: number; totals: any }> => {
    const offset = (q.page - 1) * q.limit;
    const joinConds: string[] = [];
    const params: unknown[] = [];
    let idx = 1;

    if (q.region_id) { joinConds.push(`c.region_id = $${idx++}`); params.push(q.region_id); }
    if (q.date)      { joinConds.push(`c.date = $${idx++}`);      params.push(q.date); }
    if (q.from || q.to) { joinConds.push(dateFilter(q.from, q.to, 'c.date')); }
    ...
    const [rows, count] = await Promise.all([
      ...
      db.query<{ count: string }>(`SELECT COUNT(*) FROM regions ${regionWhere}`, q.region_id ? [q.region_id] : []),
    ]);
  ...
  findById: async (id: string): Promise<Crime | null> => {
    const r = await db.query<Crime>('SELECT * FROM crimes WHERE id = $1', [id]);
    return r.rows[0] || null;
  },

  create: async (dto: CreateCrimeDtoType): Promise<Crime> => {
    const r = await db.query<Crime>(
      `INSERT INTO crimes
      ...
      [dto.region_id,dto.total_crimes,dto.minor_crimes,dto.medium_crimes,dto.serious_crimes,
  ...
  update: async (id: string, dto: UpdateCrimeDtoType): Promise<Crime | null> => {
    const fields = Object.keys(dto);
    if (!fields.length) return crimeRepository.findById(id);
    const set = fields.map((f, i) => `${f} = $${i + 2}`).join(', ');
    const r = await db.query<Crime>(`UPDATE crimes SET ${set} WHERE id = $1 RETURNING *`, [id, ...Object.values(dto)]);
    return r.rows[0] || null;
  },

  delete: async (id: string): Promise<boolean> => {
    const r = await db.query('DELETE FROM crimes WHERE id = $1', [id]);
    return (r.rowCount ?? 0) > 0;
  },

  bulkCreate: async (dto: BulkCreateCrimeDtoType): Promise<{ inserted: number }> => {
    ...
    await db.query(
      `DELETE FROM crimes WHERE date = ANY($1::date[])`,
      [dates],
    );
    ...
    const placeholders = dto.items.map((item: any, i: number) => {
      const b = i * 12;
      values.push(
        item.region_id, item.total_crimes, item.minor_crimes, item.medium_crimes,
        item.serious_crimes, item.critical_crimes,
        item.total_crimes_score, item.minor_crimes_score, item.medium_crimes_score,
        item.serious_crimes_score, item.critical_crimes_score, item.date,
      );
